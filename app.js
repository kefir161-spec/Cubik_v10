...
function hideLoaderWithStartupMin(){
  ...
  var RALS=[
    ...
    ["Bion beige","#C79A63"],      // slightly darker/muted beige for previews
...
function __syncWrappers(){
  for (var i=0;i<wrappers.length;i++){
    ...
    try{
      ...
      // Специальная обработка для Zen/2
      if(isZen2LikeKind(kind)){
        var hh = getHalf(kind);
        // Добавляем небольшой запас для стабильности
...
function canPlace(center, kind, ignore){
    var h = getHalf(kind);
    // Не даём кубу проваливаться ниже пола.
    if (center.y - h.y < 0) return false;

    // Строгая проверка пересечения без сжатия бокса:
    // если новый блок геометрически заходит в любой существующий — запрещаем.
    var test = new THREE.Box3().setFromCenterAndSize(
      center,
      new THREE.Vector3(h.x * 2, h.y * 2, h.z * 2)
    );

    // Маленький допуск только против ошибок с плавающей точкой,
    // но не для того, чтобы разрешать заметное пересечение.
    ...
    for (var i = 0; i < objects.length; i++){
      ...
      var ob;
      // Для Zen/2-подобных берём реальный AABB по текущему повороту меша,
      // для остальных — предрасчитанный aabb по типу.
      if (isZen2LikeKind(okind)){
  ...
  function onMove(e){
    ...
    if(hit.object===ground){
      ...
      if(support && supportBox){
        // Ведём себя так, как будто кликнули по нижней грани этого куба.
        var axisU='x', axisV='z';
        ...
        pos.y = supportBox.min.y - h.y;

        ok = canPlace(pos, ghostType, support);
        if(ghost && ghost.userData){ ghost.userData.hitBlock = support; }
      } else {
        // Обычная логика снапа по полу
        var stx = getAxisSnapStep('x', h, ghostType);
        var stz = getAxisSnapStep('z', h, ghostType);
        pos.set(
      ...
      // Fallback: for Zen/2, if snapped-to-grid position конфликтует,
      // пробуем стыковку по самой грани (без шага), чтобы приоритет был у грани.
      if(!ok && isZen2LikeKind(ghostType)){
        var posAlt = pos.clone();
        var minU = box.min[axisU] + h[axisU];
        var maxU = box.max[axisU] - h[axisU];
        var minV = box.min[axisV] + h[axisV];
        var maxV = box.max[axisV] - h[axisV];

        posAlt[axisU] = Math.max(minU, Math.min(maxU, hit.point[axisU]));
        posAlt[axisV] = Math.max(minV, Math.min(maxV, hit.point[axisV]));

        var okAlt = canPlace(posAlt, ghostType, blk);
        if(okAlt){
          pos.copy(posAlt);
          ok = true;
        }
